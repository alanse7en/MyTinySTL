...
namespace TinySTL {
    ...
    malloc_alloc::_malloc_alloc_oom_handler = 0;
    ...
    public:
        static void* allocate(size_t n) {
            ...
            *my_free_list = ret->fre_list_link;
            return ret;
        }
    ...
    default_alloc::start_free = 0;
    default_alloc::end_free = 0;
    default_alloc::heap_size = 0;
    default_alloc::free_list[_NFREELISTS] =
    ...
    void* default_alloc::refill(size_t n) {
        ...
        *my_free_list = next_obj = (obj*)(chunk + n);
        ...
        for (i=1; ; ++i) {
            ...
            if (nobjs-1 == i) {
                curr_obj->fre_list_link = 0;
                break;
            }
            else {
                curr_obj->fre_list_link = next_obj;
            }
        }
        return ret;
    }
    ...
    char* default_alloc::chunk_alloc(size_t size, int& nobjs) {
        ...
        if (bytes_left >= total_bytes) {
            ...
            size_t bytes_to_get = 2 * total_bytes + ROUND_UP(heap_size >> 4); // copy from SGI STL, not clear why to do this
            if (bytes_left > 0) {
                obj* volatile * my_free_list = free_list
                    + FREELIST_INDEX(bytes_left);
                (obj*)start_free->fre_list_link = *my_free_list;
